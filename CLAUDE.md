...
- **`internal/ai`** — provider-neutral AI integration. `provider.go` defines the adapter contract and registry; `gemini.go` and `openai.go` implement Gemini `generateContent` and OpenAI Responses API respectively. OpenAI defaults to `gpt-5.6-luna`, reasoning `none`, and `store=false`. `blocks.go` orchestrates per-block generation: `GenerateLeafBlocks` runs SLEEP/YESTERDAY/RECOVERY in parallel via `sync.WaitGroup`, then `GenerateRecommendation` consumes their texts. Each block has its own `inputs_hash` over health inputs plus provider/model/reasoning/output-limit/prompt-revision fingerprint, so late metrics or output-affecting config changes invalidate only the required blocks. `prompt.txt` is a single template with `{{BLOCK_INSTRUCTIONS}}`; fragments live in `blockInstructions`. Adapters expose model suggestions, but Admin keeps the model field editable.
...
- **Rule-based bullets are authoritative; AI complements, never replaces.** The numeric bullets (from `internal/health/scoreSleep`/`scoreActivity`/`scoreRecovery`/`scoreCardio`) come from medical-literature heuristics. Provider-generated prose is layered underneath as `🤖 <i>...</i>` italic. If AI hallucinates, the user can sanity-check against the bullets above. Applies to all sections; `🎯 Recommendation` is AI-only because there's no rule-based equivalent.
