...
- **The rule engine is authoritative; AI only explains.** `BuildMorningInsightEvidence` selects the verdict, up to three ranked reasons, and the concrete action. The generation pipeline always uses the server's verdict reason for `SYNTHESIS` and its concrete action for `RECOMMENDATION`. The provider receives the immutable decision plus date-aligned metrics and only supplies the sleep, activity, and recovery explanations.
- **Morning reports use one scan path in both Telegram modes.** Order: Header → Today verdict → Key metrics → Why (max 3) → one AI explanation → Plan for today → one freshness line. Legacy `sendMessage` escapes dynamic text into Telegram HTML; Rich Messages uses simple headings, paragraphs, lists, emphasis, and line breaks. Do not reintroduce morning tables, blockquotes, or four repeated AI essays. Rich Messages may wrap the single AI explanation in one `<details>` element so the authoritative rule-engine evidence and action remain immediately visible.
