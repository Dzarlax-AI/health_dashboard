...
- **`internal/api`** — stable transport envelopes and the deterministic OpenAPI 3.1 generator for the client API shared by web and iOS. `contracts/openapi.json` is generated from the exact Go response types plus explicit route/auth metadata; `contracts/openapi.compat.json` is the protected compatibility baseline. `make contract-check` rejects drift and breaking response changes (removed required fields/properties, narrowed types/nullability/enums). Existing `/api/*` paths remain canonical; tenant identity is resolved only from authenticated backend context, never from client-supplied tenant/schema selectors. See `docs/CLIENT_API.md`.
