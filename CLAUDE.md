...
DATABASE_URL=postgres://... make wake-backfill-dry TZ=Europe/Belgrade FROM=2026-01-01 TO=2026-08-05 SCHEMA=health  # inspect wake_time coverage
DATABASE_URL=postgres://... make wake-backfill TZ=Europe/Belgrade FROM=2026-01-01 TO=2026-08-05 SCHEMA=health  # persist provisional wake_time rows
make docker-up        # docker compose up -d --build
make docker-down      # docker compose down
make test             # send a test POST to localhost:8080/health
make contract-generate # regenerate contracts/openapi.json from Go transport types
make contract-check    # reject generated-contract drift or breaking response changes
make web-dev           # run React/Vite workspace on 127.0.0.1:4173
make web-check         # generated contract, types, lint, tests, production build
...
make test-release-contract-helpers       # run classifier/manifest/resolver helper checks
...
- **`internal/ai`** — provider-neutral AI integration. `provider.go` defines the adapter contract and registry; `gemini.go` and `openai.go` implement Gemini `generateContent` and OpenAI Responses API respectively. OpenAI defaults to `gpt-5.6-luna`, reasoning `none`, and `store=false`. The morning insight makes exactly one structured-output call from a deterministic, date-aligned evidence packet and validates five independently usable fields: `SYNTHESIS`, `SLEEP`, `YESTERDAY`, `RECOVERY`, and `RECOMMENDATION`. The server owns the evidence and verdicts; each model field may only explain its assigned section in at most 60 words. The shared cache hash covers the exact evidence plus provider/model/reasoning/output-limit/prompt-revision fingerprint. Both adapters use bounded retry for transient failures and expose request/usage metadata without logging payloads or API keys. Adapters expose model suggestions, but Admin keeps the model field editable.

- **`internal/api`** — stable transport envelopes and the deterministic OpenAPI 3.1 generator for the client API shared by web and iOS. `contracts/openapi.json` is generated from the exact Go response types plus explicit route/auth metadata; `contracts/openapi.compat.json` is the protected compatibility baseline. `make contract-check` rejects drift and breaking client changes (removed paths/operations/parameters/success schemas or response fields, narrowed types/nullability/enums). Existing `/api/*` paths remain canonical; tenant identity is resolved only from authenticated backend context, never from client-supplied tenant/schema selectors. See `docs/CLIENT_API.md`.

- **`apps/web`** — React + TypeScript + Vite workspace for the incremental browser migration. It generates client types from `contracts/openapi.json`, uses same-origin `/api/*` requests, and owns reusable localisation, loading/error/freshness states, design tokens, surfaces, score cards, and deterministic SVG gauge primitives. `src/assets/morning-meadow-hero.webp` is the source-owned approved hero asset and is emitted with a content hash; no runtime CDN is used. `apps/web/Dockerfile` builds the workspace with pinned Node/pnpm inputs and copies only `dist/` into a pinned unprivileged Nginx runtime. The container owns static files and `/healthz`, returns 404 for unknown paths, and never proxies `/api/*`; the future production reverse proxy keeps API and legacy routes on the backend. The fixture route is development/test-only and must not be enabled in production builds. Production routing remains owned by later migration issues, so do not remove or bypass the Go UI yet.

- **`internal/ui`** — web dashboard SPA at `/`. Auth: Authentik ForwardAuth (headers `X-authentik-username`/`X-authentik-email`) or username+password cookie. `guard()` in `handler.go` checks ForwardAuth first, then API key, then session cookie. On ForwardAuth success, issues a 30-day local `auth` cookie (`username|passwordHash`) so sessions survive Authentik token expiry. Login page at `/login`. `/settings` (all users): Telegram config, import. `/admin` (admin-only): cache/backfill, AI provider config, user management. AI provider settings are installation-wide; saved keys are never returned to the browser, inactive providers retain their settings, and switching has no automatic cross-provider fallback. API endpoints: `/api/dashboard`, `/api/metrics`, `/api/metrics/data`, `/api/metrics/latest`, `/api/metrics/range`, `/api/health-briefing`, `/api/ai-briefing`, `/api/readiness-history`, `/api/settings`, `/api/import/upload`, `/api/import/status`, `/api/admin/status`, `/api/admin/backfill`, `/api/admin/settings`, `/api/admin/gaps`, `/api/admin/ai-models`, `/api/admin/users`. The entire frontend is embedded Go strings in `internal/ui/` (template, scripts, styles). Uses Chart.js 4 from CDN. **Static assets** (`static/app.js`, `static/charts.js`) embedded via `embed.FS`. Templates emit URLs as `/static/<file>?v=<hash>` where the hash is computed at process start over the embedded contents (`computeStaticVer` in `render.go`); `BasePage.StaticVer` carries it into every page render. `serveStatic` returns `Cache-Control: public, max-age=31536000, immutable` on a matching `?v=` and the previous 1h policy otherwise — so each deploy invalidates the cached bundle without a hard-refresh. All `lang` query inputs are clamped to `en/ru/sr` via `supportedLang()` so junk values cannot pollute caches.
...
- **`internal/tenants`** — tenant registry routing, durable provisioning, and database isolation. In isolated mode each active tenant authenticates with a UUID-derived PostgreSQL LOGIN role; `db_isolation_ready` must be true before `tenants.Manager` opens a request-serving runtime pool. `AdminProvisioner` intentionally uses short-lived pre-activation connections authenticated as the future tenant role to build the schema and prove ownership/ACL isolation; these are provisioning connections, never serve requests, and must not be replaced with the registry connection. Administrative provisioning/migration authority, registry authority, and tenant request pools use separate connection classes. Existing-install cutover, secret rotation, and rollback are operated through `cmd/tenant_isolation`; see `docs/TENANT_ISOLATION_RUNBOOK.md`.
  ...
  **Tenant schema release contract:** every active isolated tenant must match `storage.SchemaContractVersion` and `storage.SchemaContractChecksum()` in both the registry and its permanent `__tenant_identity` row. Provisioning and upgrades share the same deterministic contract. `tenant_isolation --mode migrate-contract --all --confirm` performs the additive fleet migration under an exclusive registry advisory lock; `--mode audit --all` verifies exact registry/marker/role sets, owners and ACLs, and all-pairs denial probes. Production deployment uses `scripts/deploy-tenant-schema-gate.sh`; never bypass its pre-stop audit, stopped-service migration, pre/post-start audits, or immutable-image checks. Index builds that may block tenant writes belong in `MigrateSchemaContractContext`, not runtime `EnsureSchemaContractContext`; the former runs only inside the stopped-service fleet gate or before a newly provisioned tenant is activated.
...
- **`internal/storage`** — PostgreSQL via `jackc/pgx/v5` connection pool. Tables: `health_records`, `metric_points`, three pre-aggregated cache tables (`minute_metrics`, `hourly_metrics`, `daily_scores`), `settings` (key-value store for Telegram config), `ai_briefing_blocks` (the five aligned AI fields keyed by lang/date/block and one exact shared `inputs_hash`; mismatched legacy leaves are suppressed), and `ai_briefings` (kept as the morning-report `sent_at` lock only — content lives in `ai_briefing_blocks`). Also includes `admin.go` (data gap detection), `settings.go` (notification + AI config persistence), `ai_briefing.go` (legacy `sent_at` CRUD), `ai_briefing_blocks.go` (cache CRUD + idempotent migration from legacy `ai_briefings.insight`), `ai_orchestrator.go` (`EnsureTodayAIInsight` + `EnsureTodayAIInsightAsync` with single-flight `aiRegenInFlight sync.Map` and 5-min failure backoff `aiRegenLastFailAt`), `derived_metrics.go` (typed registry-backed canonical derived values and feedback), `wake_detection.go` (real-time `wake_time` confidence and historical backfill), and `typical_wake.go` (canonical wake median with a legacy fallback for adaptive `MorningCapTime`). Schema managed externally via `init.sql` except explicitly auto-created contract tables, including AI briefing and derived-metric tables.
...
ai_briefing_blocks — AI cache, PRIMARY KEY (date, lang, block).
                     One structured provider call writes aligned SYNTHESIS /
                     SLEEP / YESTERDAY / RECOVERY / RECOMMENDATION rows with
                     the same inputs_hash = sha256 over exact date-aligned
                     evidence plus the generation fingerprint. Reads only
                     combine leaves whose hash matches SYNTHESIS; 'legacy'
                     marks rows migrated from the old ai_briefings.insight
                     blob. Auto-created via
                     EnsureAIBriefingBlocksTable() on startup.
derived_metrics    — canonical service-derived metrics, PRIMARY KEY
                     (metric_name, metric_date), with exactly one typed value.
                     `wake_time` is the first registered metric and records
                     provisional/final state, formula version, inputs hash,
                     calculation/finalization timestamps, and diagnostics.
derived_metric_feedback — calibration labels, PRIMARY KEY
                     (metric_name, metric_date, channel). Stores proposed
                     values, the first idempotent response, Telegram message
                     id, and prompt/answer timestamps. The settings key
                     `wake_feedback_enabled=false` disables new wake prompts.
...
| `AI_PROVIDER` | `gemini` | Active AI adapter: `gemini` or `openai`; overridable installation-wide in Admin UI |
| `AI_MAX_OUTPUT_TOKENS` | `5000` | Shared max output tokens for the active AI provider |
...
| `GEMINI_MAX_TOKENS` | `5000` | Legacy output-token fallback; prefer `AI_MAX_OUTPUT_TOKENS` |
| `OPENAI_API_KEY` | — | OpenAI API key for direct Responses API access |
| `OPENAI_MODEL` | `gpt-5.6-luna` | OpenAI model; overridable in Admin UI |
| `OPENAI_REASONING_EFFORT` | `none` | OpenAI reasoning effort: none/low/medium/high/xhigh/max |
...
- **The rule engine is authoritative; AI only explains.** `BuildMorningInsightEvidence` selects the verdict, up to three ranked reasons, and the concrete action. The provider receives that immutable decision plus date-aligned metrics and returns one short `SYNTHESIS` explanation; it cannot replace the action or invent a separate recommendation.
- **Morning reports use one scan path in both Telegram modes.** Order: Header → Today verdict → Key metrics → Why (max 3) → one AI explanation → Plan for today → one freshness line. Legacy `sendMessage` escapes dynamic text into Telegram HTML; Rich Messages uses simple headings, paragraphs, lists, emphasis, and line breaks. Do not reintroduce morning tables, blockquotes, details, or four repeated AI essays.
...
**Smart-retry gate** (`internal/storage/wake_detection.go::ComputeMorningWakeStatus`): both morning paths use the same canonical detector. It prefers the end (`start + qty`) of the latest detailed non-midnight sleep/awake stage, falls back to raw `sleep_total`, waits for 20 minutes of ingest quiet, and confirms wake via ≥100 post-candidate steps or versioned timeouts against the personal median. The old all-day 300-step proxy is not used. Scheduler polling still force-sends at `MorningCapHour` with a localized stale banner; durable report dedup remains `db.HasSentMorningReport(today)`. Both paths route through `notify.DecideMorningAction`. After delivery, medium/low-confidence wake feedback is asked in Telegram; high-confidence control prompts are limited to one per 7 days, and a wake prompt suppresses the same-morning context prompt to avoid two inline questions.
