...
| `chronic_load / chronic_label` | **closed: event_base_rate production layer** | `event_base_rate` precision@R=0.5 = 0.850 (split-local), CI [0.682, 1.000] | L2 logistic α=0.01 precision@R=0.5 = 0.750, CI [0.593, 1.000], AUC 0.936 vs floor AUC 0.794 | model lower CI below floor upper CI; CIs overlap. Model has real ranking signal (AUC +0.14) but precision@R=0.5 saturates at high prevalence (30% test base rate) so the floor already scores near-perfectly. Walk-forward shows precision ≈ 1.0 on most denser months. | re-evaluate at a higher-information threshold (lower target recall, or top-k precision) where the AUC advantage may translate to a precision lift over base rate |
