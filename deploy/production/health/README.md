...
- only `/`, `/sleep`, `/activity`, `/cardio`, `/recovery`, and `/assets/*` may move to the frontend.
...
- `cutover`: `/`, `/sleep`, `/activity`, `/cardio`, `/recovery`, and `/assets/*` default to the frontend.
...
6. `/metrics`, `/settings`, `/admin`, `/static/*`, and `/legacy` reach Go; `/sleep`, `/activity`, `/cardio`, and `/recovery` reach React.
