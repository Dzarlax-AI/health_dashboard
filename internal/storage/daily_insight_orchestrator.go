...
// EnsureDailyInsightNarrativeSlotsAsync schedules independently refreshable
// overall, sleep, recovery and energy prose. Each worker receives and leases
// only its own closed claim packet, so a late metric cannot cause unrelated
// cards to flicker or pay for another provider call.
func (s *DB) EnsureDailyInsightNarrativeSlotsAsync(snapshot *health.DailyInsightSnapshot, aiCfg AIConfig, lang string) bool {
	if snapshot == nil || !aiCfg.Enabled() || !TodayInsightsB1Enabled(s) || !TodayInsightsB1ApprovedForConfig(s, aiCfg) {
		return false
	}
...
func (s *DB) EnsureDailyInsightNarrativeSlot(ctx context.Context, snapshot *health.DailyInsightSnapshot, aiCfg AIConfig, lang, slot, materialHash, providerFingerprint string) error {
	...
	if !aiCfg.Enabled() || !TodayInsightsB1Enabled(s) || !TodayInsightsB1ApprovedForConfig(s, aiCfg) || len(input.Slot.Claims) == 0 {
		return nil
	}
