...
The backend build is pure Go (no CGO). Uses `jackc/pgx/v5` for PostgreSQL. Dependencies resolve from `go.mod` / `go.sum`; `vendor/` is intentionally not committed. The frontend uses Node.js 24+, the root pnpm workspace, React, TypeScript, and Vite. Run `pnpm install --frozen-lockfile` from the repository root after a clean checkout. Docker images are built via GitHub Actions.
