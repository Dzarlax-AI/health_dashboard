...
case "$mode" in
  canary)
    enabled=true
    root_rule="Host(\`$host\`) && (Path(\`/\`) || Path(\`/sleep\`) || Path(\`/activity\`) || Path(\`/cardio\`) || Path(\`/recovery\`) || Path(\`/energy\`)) && HeaderRegexp(\`Cookie\`, \`(^|;[ ]*)health_frontend_canary=1(;|[ ]*\$)\`)"
    assets_rule="Host(\`$host\`) && PathPrefix(\`/assets/\`) && HeaderRegexp(\`Cookie\`, \`(^|;[ ]*)health_frontend_canary=1(;|[ ]*\$)\`)"
    ;;
  cutover)
    enabled=true
    root_rule="Host(\`$host\`) && (Path(\`/\`) || Path(\`/sleep\`) || Path(\`/activity\`) || Path(\`/cardio\`) || Path(\`/recovery\`) || Path(\`/energy\`))"
