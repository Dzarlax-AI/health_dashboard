...
write_release() {
  ...
  case "$mode" in
    canary)
      root_rule='Host(`health.example.com`) && (Path(`/`) || Path(`/sleep`) || Path(`/activity`) || Path(`/cardio`) || Path(`/recovery`) || Path(`/energy`)) && HeaderRegexp(`Cookie`, `(^|;[ ]*)health_frontend_canary=1(;|[ ]*$)`)'
      assets_rule='Host(`health.example.com`) && PathPrefix(`/assets/`) && HeaderRegexp(`Cookie`, `(^|;[ ]*)health_frontend_canary=1(;|[ ]*$)`)'
      ;;
    cutover)
      root_rule='Host(`health.example.com`) && (Path(`/`) || Path(`/sleep`) || Path(`/activity`) || Path(`/cardio`) || Path(`/recovery`) || Path(`/energy`))'
