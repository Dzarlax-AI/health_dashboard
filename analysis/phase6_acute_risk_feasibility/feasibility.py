#!/usr/bin/env python3
"""Phase 1 step 2.5 — Acute Risk `event_t1_t3` classifier feasibility.

Question: does a regularized logistic regression over the Phase 0
Acute Risk own-features beat the `event_base_rate` naive floor on
...
## Running
    ...
    PSQL=/path/to/psql.exe PYTHONUTF8=1 python \\
        analysis/phase6_acute_risk_feasibility/feasibility.py \\
        > READINESS_REDESIGN_PHASE1_ACUTE_RISK_FEASIBILITY.md
...
SUB_SCORE = "acute_risk"
TARGET_KIND = "event_t1_t3"

# Feature keys from internal/storage/acute_risk_writer.go
# (`acuteRiskFeatures` struct). Booleans loaded as 0/1 floats; ints
# load as floats. Many today-/sd-/z- fields are `omitempty` pointers,
# so rows with NULL on any feature key drop out via to_matrix's
# complete-cases filter (same as phase4/phase5).
FEATURE_KEYS = [
    "hrv_today",
    "rhr_today",
    "hrv_mean_45d",
    ...
    "paired_count_to_t",
    "warmup_met",
    "hrv_eligible_count_45d",
    "rhr_eligible_count_45d",
]
...
def load_rows() -> list[Row]:
    """Eligible target + features for event_t1_t3, test
...
def load_event_base_rate() -> dict[date, float]:
    """event_base_rate naive baseline predictions for event_t1_t3."""
    sql = f"""
...
def render(primary: dict, sensitivity: dict, n_dropped: int) -> str:
    out = []
    out.append(f"# Readiness Redesign — Phase 1 Acute Risk `{TARGET_KIND}` Classifier Feasibility")
    out.append("")
    out.append("Auto-generated by `analysis/phase6_acute_risk_feasibility/feasibility.py`. Re-run any time.")
    out.append("")
    ...
    out.append("- **Features**: Acute Risk own-features from `feature_snapshots` (no cross-sub_score signals on this iteration).")
    ...
    if model_p is None or floor_p is None:
        ...
        out.append(
            ...
            f"different event_t1_t3 criterion, or accept event_base_rate as the deployable layer."
        )
    else:
        out.append(
    ...
    out.append("")
    scope_pos = primary['test_positives']
    scope_range = f"{primary['test_range'][0]} → {primary['test_range'][1]}"
    out.append(
        f"**Scope of the verdict.** This is a statement about the **current chronological test tail** "
        f"({scope_range}, {scope_pos} positives in {primary['n_test']} rows), not a global claim about the `{TARGET_KIND}` label. "
        f"If the walk-forward section below shows materially higher precision on earlier months with denser positives, "
        f"that points to seasonality or regime dependence rather than a useless label. "
        f"The production decision is governed by the primary chronological split because that is the tail the live system would score; "
        f"revisit when more positives accumulate."
    )
    out.append("")
